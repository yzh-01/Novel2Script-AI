...
export const ScreenplayPreview = memo(function ScreenplayPreview({ yaml }: ScreenplayPreviewProps) {
  ...
  if (!screenplay) {
    return (
      <div className="flex h-full items-center justify-center rounded-lg border bg-gray-50 p-6 text-center text-sm text-gray-400">
        {yaml.trim()
        ...
        <div className="mt-2 grid grid-cols-2 gap-1 rounded bg-gray-50 p-2 text-xs">
          {screenplay.characters.map(ch => (
            <span key={ch.id} className="flex items-center gap-1 px-1 py-0.5">
              ...
              {ch.name}
              <span className="text-gray-400">({ch.category === 'main' ? '主' : ch.category === 'supporting' ? '配' : '其他'})</span>
            </span>
          ))}
        </div>
      </details>
        ...
        {screenplay.scenes.map(scene => (
          <div key={scene.id} className="border-t pt-4">
            ...
            <h3 className="mb-2 font-semibold text-gray-800">
              ...
              {scene.heading.extra && <span className="text-gray-400">（{scene.heading.extra}）</span>}
            </h3>

            {scene.summary && (
              <p className="mb-2 text-xs italic text-gray-400">{scene.summary}</p>
            )}
            ...
            <div className="space-y-2">
              {scene.blocks.map((block, i) => {
                  ...
                  return (
                    <div key={i} className="ml-6 border-l-2 border-amber-200 pl-3">
                      <p className="text-sm font-medium text-amber-900">
                        {characterName}
                        {block.parenthetical && (
                          <span className="ml-2 font-normal text-gray-400">{block.parenthetical}</span>
                        )}
                        {block.delivery === 'voice_over' && <span className="ml-1 text-xs text-gray-400">(V.O.)</span>}
                        {block.delivery === 'off_screen' && <span className="ml-1 text-xs text-gray-400">(O.S.)</span>}
                      </p>
                      ...
                      {block.emotion && (
                        <p className="text-xs text-gray-400">情感：{block.emotion}</p>
                      )}
                    </div>
                  );
                }
                if (block.type === 'transition') {
                  return (
                    <p key={i} className="text-right text-xs font-medium uppercase tracking-wider text-gray-400">
                      {block.text}
                    </p>
                  );
                }
